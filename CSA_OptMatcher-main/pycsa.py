...
Based on the code at https://github.com/structurely/csa
Modified by Fabio Tamburini, https://github.com/ftamburin
"""
...
class CoupledAnnealer(object):
	...
	**Parameters**:
	...
	def __init__(self, target_function, probe_function,
		...
		# Initialize energies.
		self.probe_energies = self.current_energies = [1e10] * self.m
	...
	def __status_check(self, k, energy, temps=None, start_time=None):
		...
		if start_time:
			...
			print("Step {:6d}, Energy {:,.8f}, Elapsed time {:,.2f} secs"
				  .format(k, energy, elapsed), file=sys.stderr)
		else:
			print("Step {:6d}, Energy {:,.8f}".format(k, energy), file=sys.stderr)
		if temps:
			print("Updated acceptance temp {:,.15f}".format(temps[0]), file=sys.stderr)
			print("Updated generation temp {:,.15f}".format(temps[1]), file=sys.stderr)
	...
	def anneal(self):
		...
		if (self.processes > 1):
			print('PERFORMING PARALLEL ANNEALING', file=sys.stderr)
			update_func = self.__update_state
		else:
			print('PERFORMING SERIAL ANNEALING', file=sys.stderr)
...
def worker_probe(annealer, i):
	...
	if (random.random() < annealer.qa):
		probe = annealer.probe_function(state, annealer.tgen, qa=True)
	else:
		probe = annealer.probe_function(state, annealer.tgen, qa=False)
